...
public enum GameVersion {
    ...
    private static final Map<String, GameVersion> ENUM_MAP;

    private final String index;

    private GameVersion(String index) {
        this.index = index;
    }
    ...
    static {
        Map<String, GameVersion> map = new ConcurrentHashMap<>();
        for (GameVersion instance : GameVersion.values()) {
            map.put(instance.getIndex(), instance);
        }
        ENUM_MAP = Collections.unmodifiableMap(map);
    }

    public static GameVersion get(String index) {
